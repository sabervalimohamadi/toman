...
export function payByCode(driverCode: string, amount: number): Promise<{ message: string }> {
  return client.post<{ message: string }>('/payment/pay/code', { driverCode, amount }).then((r) => r.data)
}

export function getPaymentHistory(page: number, limit: number, status?: string): Promise<IPaginated<IPayment>> {
  return client.get<IPaginated<IPayment>>('/payment/history', { params: { page, limit, status } }).then((r) => r.data)
}
