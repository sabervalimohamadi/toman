...
export const useAuthStore = defineStore('auth', () => {
  ...
  const token = ref<string | null>(storage.get('taxi_token'))
  const user = ref<IUser | null>(token.value ? decodeToken(token.value) : null)
  ...
  async function verifyOtp(phone: string, code: string) {
    ...
    try {
      ...
      user.value = res.user
      storage.set('taxi_token', res.accessToken)
